#if SERVER

import CSSBuilder
import DesignTokens
import HTMLBuilder
import SVGBuilder
import WebTypes
import DOMBuilder

/// An animated filled chevron SVG that morphs between collapsed (v down) and
/// expanded (^ up) states via SMIL `<animate>` on the polygon `points` attribute.
/// Used for table sort indicators and group expand/collapse toggles.
///
/// To trigger the animation from JS/CLIENT:
/// ```js
/// let el = document.querySelector('#my-id-up-down-chevron animate');
/// el.beginElement();
/// setTimeout(() => {
///   let from = el.getAttribute('from'), to = el.getAttribute('to');
///   el.parentElement.setAttribute('points', to);
///   el.setAttribute('from', to);
///   el.setAttribute('to', from);
/// }, 210);
/// ```
public struct AnimatedUpDownChevronView: HTMLContent {
	let id: String
	let expanded: Bool
	let width: Length
	let height: Length
	let `class`: String

	public init(
		id: String,
		expanded: Bool = false,
		width: Length = px(20),
		height: Length = px(20),
		class: String = ""
	) {
		self.id = id
		self.expanded = expanded
		self.width = width
		self.height = height
		self.class = `class`
	}

	public func render() -> DOMNode {
		// MARK: - Chevron Geometry (20x20 viewBox)
		// Collapsed (v down): (2.5, 4.75) (10, 12.25) (17.5, 4.75) (19, 6.25) (10, 15.25) (1, 6.25)
		// Expanded (^ up):   (2.5, 15.25) (10, 7.75) (17.5, 15.25) (19, 13.75) (10, 4.75) (1, 13.75)

		return svg {
			if expanded {
				polygon {
					animate()
						.attributeName(.points)
						.from((2.5, 15.25), (10, 7.75), (17.5, 15.25), (19, 13.75), (10, 4.75), (1, 13.75))
						.to((2.5, 4.75), (10, 12.25), (17.5, 4.75), (19, 6.25), (10, 15.25), (1, 6.25))
						.dur(ms(200))
						.fill(.freeze)
						.begin(.indefinite)
				}
				.points((2.5, 15.25), (10, 7.75), (17.5, 15.25), (19, 13.75), (10, 4.75), (1, 13.75))
				.fill(.currentColor)
			} else {
				polygon {
					animate()
						.attributeName(.points)
						.from((2.5, 4.75), (10, 12.25), (17.5, 4.75), (19, 6.25), (10, 15.25), (1, 6.25))
						.to((2.5, 15.25), (10, 7.75), (17.5, 15.25), (19, 13.75), (10, 4.75), (1, 13.75))
						.dur(ms(200))
						.fill(.freeze)
						.begin(.indefinite)
				}
				.points((2.5, 4.75), (10, 12.25), (17.5, 4.75), (19, 6.25), (10, 15.25), (1, 6.25))
				.fill(.currentColor)
			}
		}
		.class(`class`.isEmpty ? "animated-up-down-chevron" : "animated-up-down-chevron \(`class`)")
		.id("\(id)-up-down-chevron")
		.width(width)
		.height(height)
		.viewBox(0, 0, 20, 20)
		.xmlns("http://www.w3.org/2000/svg")
		.render()
	}
}

#endif
